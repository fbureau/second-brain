...
```
_CLAUDE.md       ← this file — read first
TODO.md          ← consolidated action list (generated by task-roundup, two-way sync)
...
### 3.7 Append-only on sensitive notes
On `02-people/`, `05-decisions/`, and the `## Sources` section of any wiki page: NEVER
overwrite — add entries to "Timeline" or "Updates" sections with a timestamp. Elsewhere,
legitimate in-place edits (wiki enrichment, project status, checkbox sync, re-languaging)
are allowed, but a dated correction always beats a silent rewrite.
...
transcript-source: verbatim | summary-fallback   # which Drive tab/file was ingested (since v4.0)
...
needs-review: true | false                        # true if auto and not yet reviewed, or summary-fallback
ai-first: true
```
...
are maintained automatically by `knowledge-build` (curator mode); see Section 9.

### Other system types (no full schema needed)
- `type: decision-challenge` — red-team analyses, `05-decisions/YYYY-MM-DD-challenge-<slug>.md` (challenge-decision).
- `type: weekly-review` — `01-daily/YYYY-WW-weekly.md` (daily-brief weekly mode).
- `type: todo-dashboard` — the vault-root `TODO.md` (task-roundup).
- `type: profile` — `00-inbox/MY-PROFILE.md`.
- `type: backfill-report` — the kickstart-backfill meta report in `06-knowledge/`.
...
`## Execution plan`, daily `## Pending follow-ups`, people timeline checkbox
`- [ ] Follow-up:` lines, project action lines, braindump `## Suggested follow-up`).
The `task-roundup` skill consolidates the ones the user owns into a single
**`TODO.md` at the vault root** and keeps the checkboxes synced.
...
1. Detects new meeting transcripts → invokes `meeting-ingest` automatically (except sensitive 1-1s → flag for manual validation). Transcript-first: the verbatim transcript tab is read, never the lossy summary tab, unless the transcript is missing (then `confidence: medium`, `needs-review: true`).
...
5. Detects `05-decisions/` notes whose `status:` flipped to `reversed` since the last brief → invokes `challenge-decision` in postmortem mode (analysis + manifest only; cross-vault edits always await human approval, output flagged `needs-review: true`).
6. Generates the usual synthetic brief.
