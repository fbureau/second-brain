...
enforces the note contract in code — 26 tools, 13 skills, optional passive recall, cron jobs.
Nothing changes for Claude Cowork / Claude Code users: additive, **no vault migration**. One
security fix is worth the upgrade on its own.
...
```
Pick a model that does reliable function calling with a context window of at least 32k
(Ollama defaults to 4k — raise `num_ctx`). Full guide: `docs/HERMES.md`.

**3. (Optional) Turn on passive recall**
```bash
hermes config set memory.provider sb_vault
```
Read-only: it never writes to the vault. Only one external memory provider can be active at a
time, so this replaces any other you had configured.

**4. (Optional) Register the scheduled jobs**
```bash
...
**5. (Optional) Add `AGENTS.md` to your vault** — `cp vault-starter/AGENTS.md /path/to/your/vault/`
so any AGENTS.md-aware agent reads `_CLAUDE.md` first. Harmless for Claude.

**6. Check it on your machine**
```bash
...
4.2.0 fixes a path-traversal hole in the plugin: before it, a tool call with an absolute path
(`/etc/passwd`) or a `..` segment could read outside the vault, and an append could have written
outside it. Only a confused or hostile model could trigger it, but it is worth upgrading for.

### You're done when
- `hermes plugins list` shows `second_brain` enabled, and `/braindump …` creates a committed note
  in `00-inbox/`.
- `/recall what do I know about X` answers with citations, or says plainly that the vault does not know.
